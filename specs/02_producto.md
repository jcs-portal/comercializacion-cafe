...
| F2 | Mis datos (identidad fiscal) | Datos del caficultor como vendedor: nombre/razón social, tipo de persona (natural/jurídica), identificador fiscal (NIT o cédula), dirección, teléfono/WhatsApp. Se rellenan una sola vez. GET/PUT `/api/comercializacion/finca` (alias de `/api/finca`: es la Ficha de Productor compartida con Finanzas Cafeteros, Diagnóstico de Finca y Plan de Mejora — desde julio 2026 también se puede rellenar desde "Mi Finca" en Finanzas Cafeteros). | Tab "Mis datos" |
...
| F4 | Mis ventas | Registro de ventas: fecha, variedad, cliente (del catálogo propio), etapa de venta (E1-E6), cantidad (kg), precio, moneda (COP/USD/EUR) y coste logístico opcional. Calcula y muestra: ingreso total en COP, margen de producto y margen neto (absoluto y %). Cada venta puede editarse (✎) o borrarse (✕) después de registrada; muestra fecha de registro (`registradoEn`) y, si aplica, de última edición (`actualizadoEn`). GET/POST/PUT/DELETE `/api/comercializacion/ventas`. | Tab "Ventas" |
